...
from core.clipping import compute_obb_mask, apply_mask, get_cloud_bounds
...
def benchmark_clipping_obb(n_points: int, n_iterations: int = 10):
    """Benchmark the full OBB clipping pipeline for a given point count."""
    points = np.random.rand(n_points, 3).astype(np.float64) * 100.0
    colors = np.random.randint(0, 256, (n_points, 3), dtype=np.uint8)
    
    # 45 degree rotation around Z axis + translation
    angle = np.radians(45.0)
    c, s = np.cos(angle), np.sin(angle)
    transform = np.identity(4)
    transform[:2, :2] = [[c, -s], [s, c]]
    transform[:3, 3] = [10.0, -10.0, 5.0]
...
if __name__ == '__main__':
    print("=" * 70)
    print("Tools Benchmark: Clipping Pipeline Performance (AABB vs OBB)")
    print("=" * 70)
    
    print("\n--- AABB Clipping Performance ---")
    print(f"{'Points':>10} | {'Selected':>10} | {'Selected':>11} | {'Mask (ms)':>9} | {'Apply (ms)':>10} | {'Total (ms)':>10} | {'P95 Total':>10}")
    print("-" * 85)
    ...
    print("\n--- OBB Clipping Performance ---")
    print(f"{'Points':>10} | {'Selected':>10} | {'Selected':>11} | {'Mask (ms)':>9} | {'Apply (ms)':>10} | {'Total (ms)':>10} | {'P95 Total':>10}")
    print("-" * 85)
    for n in [100_000, 500_000, 1_000_000]:
        r = benchmark_clipping_obb(n)
        print(f"{r['n_points']:>10,} | {r['selected']:>10,} | {r['selectivity']:>11} | {r['mask_ms']:>9} | {r['apply_ms']:>10} | {r['total_ms']:>10} | {r['total_p95']:>10}")
